...
var arrayContainsEquivalentObject = function(array, ob){
  var eq = require('../helpers/eq.js');
  var match = false;
  for (var i in array){
    if (eq.firstContainsSecond(array[i], ob)) match = true;
  }
  return match;
};
  ...
  if (post.mentions && Array.isArray(post.mentions)) {
    for (let m in post.mentions) {
      if (post.mentions[m].entity) {
        ...
        if (!arrayContainsEquivalentObject(references, mention)) references.push(mention);
      }
    }
  }
  if (post.references && Array.isArray(post.references)) {
    for (let r in post.refs) {
      if (post.refs[r].entity) {
        ...
        if (!arrayContainsEquivalentObject(references, ref)) references.push(ref);
      }
    }
  }
  if (post.version && post.version.parents && Array.isArray(post.version.parents)) {
    for (let p in post.version.parents) {
      if (post.version.parents[p].entity) {
        ...
        if (!arrayContainsEquivalentObject(references, parent)) references.push(parent);
      }
    }
  }
  let urlarray = [];
...
var overwriteMetaPostServers = function(userid, post){
  ...
  var sortServers = require('../helpers/sort-servers.js');
  var eq = require('../helpers/eq.js');
  ...
  if (post.content && post.content.servers && Array.isArray(post.content.servers)) {
    eachserver:
    for (let s of post.content.servers){
      if (typeof post.content.servers[s].urls !== 'object' || Object.keys(post.content.servers[s].urls).length === 0){
        post.content.servers.splice(s, 1);
        continue eachserver;
      }
      if (typeof post.content.servers[s].preference !== 'number') {
        post.content.servers.splice(s, 1);
        continue eachserver;
      }
      if (eq.firstMatchesSecond(post.content.servers[s].urls, goodserver.urls)) {
        post.content.servers.splice(s, 1);
        continue eachserver;
      }
      if (post.content.servers[s].preference === 0) post.content.servers[s].preference = 1;
    }
    sortServers(post.content.servers);
...
var savePost = function*(userid, post, mode, from, transaction){
  'use strict';
  if (typeof post.type !== 'string') throw Error('Post must include the type field');
