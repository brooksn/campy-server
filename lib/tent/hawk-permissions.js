...
var can = require('../helpers/can-i-read-write.js');
...
var getPermissions = function *(next){
  ...
  if (appid !== null) {
    ...
    var apps = yield global.knex('posts').whereRaw(typeq).whereRaw(idq);
  ...
  this.can = {
    read: function(type){
      return can.read(type, permissions);
    },
    write: function(type){
      return can.write(type, permissions);
    }
  };
  yield next;
};
