...
var posts_feed = function(method){
  var gen = function*(userid, next){
    ...
    var query = this.knex.select('json').from('posts').whereRaw(this.userid + ' = ANY(users)');
    ...
    //var posts = yield this.knex('posts').select('json').where(where).orderBy(orderby, direction).limit(limit);
